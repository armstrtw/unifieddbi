...
void PostgresConnection::connect(const char *user, const char *pass, const char *host, const char *port, const char *tty,
                                 const char *dbName, const char *options) {
...
SEXP PostgresConnection::listTables() {
  ...
  const char *query =
      "select schemaname, tablename from pg_tables where schemaname !='information_schema' and schemaname !='pg_catalog'";
...
void PostgresConnection::getOIDs(const char *tableName, vector<string> &relationNames, vector<Oid> &oids) {
  stringstream query;
  query << "SELECT attname, atttypid FROM pg_attribute WHERE attrelid = '" << tableName
        << "'::regclass and attnum > 0 and attisdropped = FALSE";
  PGresult *res = PQexec(conn_, query.str().c_str());
  if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res)) {
...
int PostgresConnection::write(const char *tableName, const vector<ColumnForWriting> &cols, const int nrows) {
  ...
  for (int i = 0; i < numCols; i++) {
    ...
    pg_col_writers.push_back(
        PostgresColumnWriter::createPostgresColumnWriter(paramTypes[i], cols[i], paramValues[i], paramLengths[i]));
  }

  // init paramFormats
  for (int i = 0; i < numCols; i++) { paramFormats[i] = pg_col_writers[i]->getFormat(); }

  res = PQprepare(conn_, prepared_stmt_name, command.c_str(), numCols, paramTypes);
  ...
  do {
    PQclear(res);
    // update pointer set
    for (int i = 0; i < numCols; i++) { pg_col_writers[i]->update(currentRow); }

    // insert
    res = PQexecPrepared(conn_, prepared_stmt_name, numCols, paramValues, paramLengths, paramFormats, resultFormat);
    ++currentRow;
  ...
  for (int i     = 0; i < numCols; i++) { delete pg_col_writers[i]; }
  deallocate_res = PQexec(conn_, (string("DEALLOCATE ") + string(prepared_stmt_name)).c_str());
  if (PQresultStatus(deallocate_res) != PGRES_COMMAND_OK) { REprintf("problem w/ DEALLOCATE on prepared statement."); }
